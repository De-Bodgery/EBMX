...
#define SINCOS_SAMPLE_RATE_HZ		2000
#define SINCOS_MIN_AMPLITUDE		1.8		// sqrt(sin^2 + cos^2) has to be larger than this
#define SINCOS_MAX_AMPLITUDE		3.9	// sqrt(sin^2 + cos^2) has to be smaller than this
...
static SerialConfig TS5700N8501_uart_cfg = { 2500000, 0,
USART_CR2_LINEN, 0 };
...
void encoder_init_as5047p_spi(void) {
	...
	TIM_TimeBaseStructure.TIM_Period = ((168000000 / 2 / AS5047_SAMPLE_RATE_HZ)
			- 1);
...
void encoder_init_mt6816_spi(void) {
	...
	TIM_TimeBaseStructure.TIM_Prescaler = 0;
	TIM_TimeBaseStructure.TIM_CounterMode = TIM_CounterMode_Up;
	TIM_TimeBaseStructure.TIM_Period = ((168000000 / 2 / MT6816_SAMPLE_RATE_HZ)
			- 1);
	TIM_TimeBaseStructure.TIM_ClockDivision = 0;
	TIM_TimeBaseStructure.TIM_RepetitionCounter = 0;
	TIM_TimeBaseInit(HW_ENC_TIM, &TIM_TimeBaseStructure);
...
void encoder_init_sincos(float s_gain, float s_offset, float c_gain,
		float c_offset, float filter_constant) {
...
float encoder_read_deg(void) {
	...
	case ENCODER_MODE_SINCOS: {
		...
		float sin = (ENCODER_SIN_VOLTS - sin_offset) * sin_gain;
		float cos = (ENCODER_COS_VOLTS - cos_offset) * cos_gain;
		...
		/*
		 ...
		 UTILS_LP_FAST(sincos_signal_above_max_error_rate, 1.0,
		 1./SINCOS_SAMPLE_RATE_HZ);
		 angle = last_enc_angle;
		 } else {
		 if (module < SQ(SINCOS_MIN_AMPLITUDE)) {
		 ++sincos_signal_below_min_error_cnt;
		 UTILS_LP_FAST(sincos_signal_low_error_rate, 1.0,
		 1./SINCOS_SAMPLE_RATE_HZ);
		 angle = last_enc_angle;
		 */

		//} else {		//correct amplitude
		/*UTILS_LP_FAST(sincos_signal_above_max_error_rate, 0.0,
		 1./SINCOS_SAMPLE_RATE_HZ);
		 UTILS_LP_FAST(sincos_signal_low_error_rate, 0.0,
		 1./SINCOS_SAMPLE_RATE_HZ);
		 */
	...
	if (palReadPad(HW_HALL_ENC_GPIO3, HW_HALL_ENC_PIN3)) {
		...
		if (index_found) {
			...
			if (cnt > (enc_counts - lim) || cnt < lim) {
				...
				bad_pulses++;
				abi_encoder_zero_identifier();	//correct the encoder
				if (bad_pulses > 5) {
					index_found = 0;
					abi_encoder_zero_identifier();	//correct the encoder
...
//angle = ((float) HW_ENC_TIM->CNT * 360.0) / (float) enc_counts;
void abi_encoder_zero_identifier(void) {
	static float absolute;
	absolute = (float) 360 - (float) utils_map(time_result, 0, 101, 0, 360);
	if (index_found == 0) {
		static unsigned int result;

		result = ((float) absolute * (float) enc_counts) / (float) 360;
		HW_ENC_TIM->CNT = result;
		index_found = 1;

	}
//commands_printf("counter value %i", HW_ENC_TIM->CNT);
}
