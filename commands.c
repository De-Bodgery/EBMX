...
static void (*volatile send_func_can_fwd)(unsigned char *data,
		unsigned int len) = 0;
...
void commands_init(void) {
	...
	chThdCreateStatic(blocking_thread_wa, sizeof(blocking_thread_wa),
	NORMALPRIO, blocking_thread, NULL);
	isInitialized = true;
}
...
void commands_process_packet(unsigned char *data, unsigned int len,
	...
	case COMM_FW_VERSION: {
		...
		strcpy((char*) (send_buffer + ind), product_serial);
		ind += strlen(product_serial) + 1;
	...
	case COMM_ERASE_NEW_APP: {
		...
		uint16_t flash_res = flash_helper_erase_new_app(
				buffer_get_uint32(data, &ind));
	...
	case COMM_WRITE_NEW_APP_DATA_ALL_CAN:
		if (packet_id == COMM_WRITE_NEW_APP_DATA_ALL_CAN_LZO) {
			...
			lzo1x_decompress_safe(send_buffer_global, len - 6, data + 4,
	...
	case COMM_WRITE_NEW_APP_DATA: {
		...
		uint16_t flash_res = flash_helper_write_new_app_data(new_app_offset,
				data + ind, len - ind);
	...
	case COMM_GET_VALUES_SELECTIVE: {
		...
		if (mask & ((uint32_t) 1 << 0)) {
			buffer_append_float16(send_buffer, mc_interface_temp_fet_filtered(),
					1e1, &ind);
		}
		if (mask & ((uint32_t) 1 << 1)) {
			buffer_append_float16(send_buffer,
					mc_interface_temp_motor_filtered(), 1e1, &ind);
		}
		...
		if (mask & ((uint32_t) 1 << 4)) {
			buffer_append_float32(send_buffer, mc_interface_read_reset_avg_id(),
					1e2, &ind);
		}
		if (mask & ((uint32_t) 1 << 5)) {
			buffer_append_float32(send_buffer, mc_interface_read_reset_avg_iq(),
					1e2, &ind);
		}
		if (mask & ((uint32_t) 1 << 6)) {
			buffer_append_float16(send_buffer,
					mc_interface_get_duty_cycle_now(), 1e3, &ind);
		}
		if (mask & ((uint32_t) 1 << 7)) {
			buffer_append_float32(send_buffer, mc_interface_get_actual_rpm(),
					1e0, &ind);
		}
		...
		if (mask & ((uint32_t) 1 << 9)) {
			buffer_append_float32(send_buffer,
					mc_interface_get_amp_hours(false), 1e4, &ind);
		}
		if (mask & ((uint32_t) 1 << 10)) { //TOTAL TRIP TIME
			buffer_append_float32(send_buffer, app_get_total_tt(), 1e0, &ind);
		}
		if (mask & ((uint32_t) 1 << 11)) {
			buffer_append_float32(send_buffer,
					mc_interface_get_watt_hours(false), 1e4, &ind);
		}
		if (mask & ((uint32_t) 1 << 12)) { //6v analog3
			buffer_append_float32(send_buffer, ENCODER_SIN_VOLTS, 1e2, &ind);
		}
		if (mask & ((uint32_t) 1 << 13)) { //THROTTLE
			buffer_append_float32(send_buffer, app_adc_get_voltage(), 1e2, //ok
					&ind);
		}
		if (mask & ((uint32_t) 1 << 14)) { //Regen
			buffer_append_float32(send_buffer, app_adc_get_voltage2(), 1e2, //ok
					&ind);
		}
		...
		if (mask & ((uint32_t) 1 << 16)) { //DIGITAL INPUTS
			...
			tmp = READ_HALL1();	//HALL1
			tmp = tmp | (READ_HALL2() << 1);	//HALL2
			tmp = tmp | (READ_HALL3() << 2);	//HALL3
			tmp = tmp | (palReadPad(ENCODER_ZERO_IO, ENCODER_ZERO_PIN) << 3);//encoder
			tmp = tmp | (palReadPad(BRAKE_IO, BRAKE_PIN) << 4);	//Brake
			tmp = tmp | (palReadPad(STAND_IO, STAND_PIN) << 5);	//Stand
			tmp = tmp | (palReadPad(TILT_IO, TILT_PIN) << 6);	//Tilt
			tmp = tmp | (palReadPad(BUTTON_SWITCH_IO, BUTTON_SWITCH_PIN) << 7);	//Switch
			buffer_append_int32(send_buffer, tmp, &ind);
		}
		...
		if (mask & ((uint32_t) 1 << 19)) {
			buffer_append_float32(send_buffer, mc_interface_read_reset_avg_vd(),
					1e3, &ind);
		}
		if (mask & ((uint32_t) 1 << 20)) {
			buffer_append_float32(send_buffer, mc_interface_read_reset_avg_vq(),
					1e3, &ind);
		}
		if (mask & ((uint32_t) 1 << 21)) {	//ODO
			buffer_append_int32(send_buffer, app_get_odo(), &ind);
		}

		if (mask & ((uint32_t) 1 << 22)) {	//6vanalog 4
			buffer_append_float32(send_buffer, ENCODER_COS_VOLTS, 1e2, &ind);

		}

		if (mask & ((uint32_t) 1 << 23)) {	//Speed
			buffer_append_float32(send_buffer, app_get_speed_km_r(), 1e2, &ind);//ok
		}

		if (mask & ((uint32_t) 1 << 24)) {//Race/Street Mode, 0 is street 1 is race
			send_buffer[ind++] = app_get_rs_mode();	//ok
			//commands_printf("rs mode: %i", app_get_rs_mode());
		}

		if (mask & ((uint32_t) 1 << 25)) {	//Assist Level
			send_buffer[ind++] = app_get_assis_lv();	//ok
			//commands_printf("assist lv: %i", app_get_assis_lv());
		}
	...
	case COMM_SET_CURRENT_BRAKE: {
		...
		mc_interface_set_brake_current(
				(float) buffer_get_int32(data, &ind) / 1000.0);
		timeout_reset();
	}
		break;
...
//#ifndef	HW_MCCONF_READ_ONLY
		...
		*mcconf = *mc_interface_get_configuration();
		...
		if (confgenerator_deserialize_mcconf(data, mcconf)) {
			...
			utils_truncate_number(&mcconf->l_abs_current_max, 0.0,
...
//#endif
	...
	case COMM_GET_MCCONF_DEFAULT: {
		...
		if (packet_id == COMM_GET_MCCONF) {
			...
			*appconf = *app_get_configuration();
			...
			if (appconf->imu_conf.mode == 0) //surron
					{
				confgenerator_set_defaults_mcconf(mcconf);
			} else if (appconf->imu_conf.mode == 1) //talaria
					{
				talaria_confgenerator_set_defaults_mcconf(mcconf);
			} else if (appconf->imu_conf.mode == 6) //kors
					{
				kors_confgenerator_set_defaults_mcconf(mcconf);
			}

			else
				//GET default only happened in vesc tool, should not happen in app
				confgenerator_set_defaults_mcconf(mcconf);
	...
	case COMM_SET_APPCONF: {
		...
		*appconf = *app_get_configuration();
		...
		if (confgenerator_deserialize_appconf(data, appconf)) {
			...
			invert_motor_direction(appconf->servo_out_enable);
			set_motor_temp_start(appconf->app_balance_conf.kd_biquad_lowpass);
			set_motor_temp_end(appconf->app_balance_conf.kd_biquad_highpass);

			if (change == 1) //motor type changed
					{
				//commands_printf("change to type: %i", appconf->imu_conf.mode);
				mc_configuration *mcconf = mempools_alloc_mcconf();
				if (appconf->imu_conf.mode == 0) //surron
						{
					confgenerator_set_defaults_mcconf(mcconf);
			...
			if (appconf->app_adc_conf.rev_button_inverted == 0) //60v stock surron
					{

				utils_truncate_number(&appconf->imu_conf.accel_confidence_decay,
						0.0,
						SURRON_NONBYPASSED_BATTERY_KW); //street throttle KW 1
				utils_truncate_number(&appconf->imu_conf.rot_roll, 0.0,
				SURRON_NONBYPASSED_BATTERY_KW); //street throttle KW 2
				utils_truncate_number(&appconf->app_ppm_conf.tc_max_diff, 0.0,
				SURRON_NONBYPASSED_BATTERY_KW); //street throttle KW 3

				utils_truncate_number(&appconf->app_chuk_conf.ramp_time_pos,
						0.0,
						SURRON_NONBYPASSED_BATTERY_KW);	//race throttle KW 1
				utils_truncate_number(
						&appconf->app_chuk_conf.smart_rev_max_duty, 0.0,
						SURRON_NONBYPASSED_BATTERY_KW);	//race throttle KW 2
				utils_truncate_number(
						&appconf->app_balance_conf.roll_steer_erpm_kp, 0.0,
						SURRON_NONBYPASSED_BATTERY_KW);	//race throttle KW 3

				utils_truncate_number(
						&appconf->app_balance_conf.torquetilt_angle_limit, 0.0,
						SURRON_NONBYPASSED_BATTERY_BA);	//street battery current

				utils_truncate_number(
						&appconf->app_balance_conf.torquetilt_on_speed, 0.0,
						SURRON_NONBYPASSED_BATTERY_BA);	//race battery current

			} else if (appconf->app_adc_conf.rev_button_inverted == 1) //60v bms surron
					{

				utils_truncate_number(&appconf->imu_conf.accel_confidence_decay,
						0.0,
						SURRON_BYPASSED_BATTERY_KW); //street throttle KW 1
				utils_truncate_number(&appconf->imu_conf.rot_roll, 0.0,
				SURRON_BYPASSED_BATTERY_KW); //street throttle KW 2
				utils_truncate_number(&appconf->app_ppm_conf.tc_max_diff, 0.0,
				SURRON_BYPASSED_BATTERY_KW); //street throttle KW 3

				utils_truncate_number(&appconf->app_chuk_conf.ramp_time_pos,
						0.0,
						SURRON_BYPASSED_BATTERY_KW);	//race throttle KW 1
				utils_truncate_number(
						&appconf->app_chuk_conf.smart_rev_max_duty, 0.0,
						SURRON_BYPASSED_BATTERY_KW);	//race throttle KW 2
				utils_truncate_number(
						&appconf->app_balance_conf.roll_steer_erpm_kp, 0.0,
						SURRON_BYPASSED_BATTERY_KW);	//race throttle KW 3

				utils_truncate_number(
						&appconf->app_balance_conf.torquetilt_angle_limit, 0.0,
						SURRON_BYPASSED_BATTERY_BA);	//street battery current

				utils_truncate_number(
						&appconf->app_balance_conf.torquetilt_on_speed, 0.0,
						SURRON_BYPASSED_BATTERY_BA);	//race battery current

			} else if (appconf->app_adc_conf.rev_button_inverted == 2) //60v talaria
					{

				utils_truncate_number(&appconf->imu_conf.accel_confidence_decay,
						0.0,
						TALARIA_NONBYPASSED_BATTERY_KW); //street throttle KW 1
				utils_truncate_number(&appconf->imu_conf.rot_roll, 0.0,
				TALARIA_NONBYPASSED_BATTERY_KW); //street throttle KW 2
				utils_truncate_number(&appconf->app_ppm_conf.tc_max_diff, 0.0,
				TALARIA_NONBYPASSED_BATTERY_KW); //street throttle KW 3

				utils_truncate_number(&appconf->app_chuk_conf.ramp_time_pos,
						0.0,
						TALARIA_NONBYPASSED_BATTERY_KW);	//race throttle KW 1
				utils_truncate_number(
						&appconf->app_chuk_conf.smart_rev_max_duty, 0.0,
						TALARIA_NONBYPASSED_BATTERY_KW);	//race throttle KW 2
				utils_truncate_number(
						&appconf->app_balance_conf.roll_steer_erpm_kp, 0.0,
						TALARIA_NONBYPASSED_BATTERY_KW);	//race throttle KW 3

				utils_truncate_number(
						&appconf->app_balance_conf.torquetilt_angle_limit, 0.0,
						TALARIA_NONBYPASSED_BATTERY_BA);//street battery current

				utils_truncate_number(
						&appconf->app_balance_conf.torquetilt_on_speed, 0.0,
						TALARIA_NONBYPASSED_BATTERY_BA);//race battery current

			} else if (appconf->app_adc_conf.rev_button_inverted == 3) //60v talaria bms
					{

				utils_truncate_number(&appconf->imu_conf.accel_confidence_decay,
						0.0,
						TALARIA_BYPASSED_BATTERY_KW); //street throttle KW 1
				utils_truncate_number(&appconf->imu_conf.rot_roll, 0.0,
				TALARIA_BYPASSED_BATTERY_KW); //street throttle KW 2
				utils_truncate_number(&appconf->app_ppm_conf.tc_max_diff, 0.0,
				TALARIA_BYPASSED_BATTERY_KW); //street throttle KW 3

				utils_truncate_number(&appconf->app_chuk_conf.ramp_time_pos,
						0.0,
						TALARIA_BYPASSED_BATTERY_KW);	//race throttle KW 1
				utils_truncate_number(
						&appconf->app_chuk_conf.smart_rev_max_duty, 0.0,
						TALARIA_BYPASSED_BATTERY_KW);	//race throttle KW 2
				utils_truncate_number(
						&appconf->app_balance_conf.roll_steer_erpm_kp, 0.0,
						TALARIA_BYPASSED_BATTERY_KW);	//race throttle KW 3

				utils_truncate_number(
						&appconf->app_balance_conf.torquetilt_angle_limit, 0.0,
						TALARIA_BYPASSED_BATTERY_BA);	//street battery current

				utils_truncate_number(
						&appconf->app_balance_conf.torquetilt_on_speed, 0.0,
						TALARIA_BYPASSED_BATTERY_BA);	//race battery current

			} else if (appconf->app_adc_conf.rev_button_inverted == 4) //60v ebmx stock
					{

				utils_truncate_number(&appconf->imu_conf.accel_confidence_decay,
						0.0,
						EBMX_16S_BATTERY_KW); //street throttle KW 1
				utils_truncate_number(&appconf->imu_conf.rot_roll, 0.0,
				EBMX_16S_BATTERY_KW); //street throttle KW 2
				utils_truncate_number(&appconf->app_ppm_conf.tc_max_diff, 0.0,
				EBMX_16S_BATTERY_KW); //street throttle KW 3

				utils_truncate_number(&appconf->app_chuk_conf.ramp_time_pos,
						0.0,
						EBMX_16S_BATTERY_KW);	//race throttle KW 1
				utils_truncate_number(
						&appconf->app_chuk_conf.smart_rev_max_duty, 0.0,
						EBMX_16S_BATTERY_KW);	//race throttle KW 2
				utils_truncate_number(
						&appconf->app_balance_conf.roll_steer_erpm_kp, 0.0,
						EBMX_16S_BATTERY_KW);	//race throttle KW 3

				utils_truncate_number(
						&appconf->app_balance_conf.torquetilt_angle_limit, 0.0,
						EBMX_16S_BATTERY_BA);	//street battery current

				utils_truncate_number(
						&appconf->app_balance_conf.torquetilt_on_speed, 0.0,
						EBMX_16S_BATTERY_BA);	//race battery current

			} else if (appconf->app_adc_conf.rev_button_inverted == 5) //72v (20s) EBMX Stock Harness
					{

				utils_truncate_number(&appconf->imu_conf.accel_confidence_decay,
						0.0,
						EBMX_20S_BATTERY_KW); //street throttle KW 1
				utils_truncate_number(&appconf->imu_conf.rot_roll, 0.0,
				EBMX_20S_BATTERY_KW); //street throttle KW 2
				utils_truncate_number(&appconf->app_ppm_conf.tc_max_diff, 0.0,
				EBMX_20S_BATTERY_KW); //street throttle KW 3

				utils_truncate_number(&appconf->app_chuk_conf.ramp_time_pos,
						0.0,
						EBMX_20S_BATTERY_KW);	//race throttle KW 1
				utils_truncate_number(
						&appconf->app_chuk_conf.smart_rev_max_duty, 0.0,
						EBMX_20S_BATTERY_KW);	//race throttle KW 2
				utils_truncate_number(
						&appconf->app_balance_conf.roll_steer_erpm_kp, 0.0,
						EBMX_20S_BATTERY_KW);	//race throttle KW 3

				utils_truncate_number(
						&appconf->app_balance_conf.torquetilt_angle_limit, 0.0,
						EBMX_20S_BATTERY_BA);	//street battery current

				utils_truncate_number(
						&appconf->app_balance_conf.torquetilt_on_speed, 0.0,
						EBMX_20S_BATTERY_BA);	//race battery current

			} else if (appconf->app_adc_conf.rev_button_inverted == 6) //82v (22s) EBMX High Power Harness
					{

				utils_truncate_number(&appconf->imu_conf.accel_confidence_decay,
						0.0,
						EBMX_22S_BATTERY_HP_KW); //street throttle KW 1
				utils_truncate_number(&appconf->imu_conf.rot_roll, 0.0,
				EBMX_22S_BATTERY_HP_KW); //street throttle KW 2
				utils_truncate_number(&appconf->app_ppm_conf.tc_max_diff, 0.0,
				EBMX_22S_BATTERY_HP_KW); //street throttle KW 3

				utils_truncate_number(&appconf->app_chuk_conf.ramp_time_pos,
						0.0,
						EBMX_22S_BATTERY_HP_KW);	//race throttle KW 1
				utils_truncate_number(
						&appconf->app_chuk_conf.smart_rev_max_duty, 0.0,
						EBMX_22S_BATTERY_HP_KW);	//race throttle KW 2
				utils_truncate_number(
						&appconf->app_balance_conf.roll_steer_erpm_kp, 0.0,
						EBMX_22S_BATTERY_HP_KW);	//race throttle KW 3

				utils_truncate_number(
						&appconf->app_balance_conf.torquetilt_angle_limit, 0.0,
						EBMX_22S_BATTERY_HP_BA);	//street battery current

				utils_truncate_number(
						&appconf->app_balance_conf.torquetilt_on_speed, 0.0,
						EBMX_22S_BATTERY_HP_BA);	//race battery current

			}

			utils_truncate_number(&appconf->imu_conf.mahony_kp, 0.0,
			MCCONF_L_CURRENT_MAX); //street throttle current 1
			utils_truncate_number(&appconf->imu_conf.rot_pitch, 0.0,
			MCCONF_L_CURRENT_MAX); //street throttle torque 2
			utils_truncate_number(&appconf->app_ppm_conf.hyst, 0.0,
			MCCONF_L_CURRENT_MAX); //street throttle torque 3
			utils_truncate_number(&appconf->app_chuk_conf.ramp_time_neg, 0.0,
			MCCONF_L_CURRENT_MAX);	//race throttle torque 1
			utils_truncate_number(&appconf->app_chuk_conf.smart_rev_ramp_time,
					0.0, MCCONF_L_CURRENT_MAX);	//race throttle torque 2
			utils_truncate_number(&appconf->app_balance_conf.yaw_current_clamp,
					0.0, MCCONF_L_CURRENT_MAX);	//race throttle torque 3

			utils_truncate_number(&appconf->imu_conf.mahony_kp, 0.0,
					get_motor_current_limit()); //street throttle current 1
			utils_truncate_number(&appconf->imu_conf.rot_pitch, 0.0,
					get_motor_current_limit()); //street throttle torque 2
			utils_truncate_number(&appconf->app_ppm_conf.hyst, 0.0,
					get_motor_current_limit()); //street throttle torque 3
			utils_truncate_number(&appconf->app_chuk_conf.ramp_time_neg, 0.0,
					get_motor_current_limit());	//race throttle torque 1
			utils_truncate_number(&appconf->app_chuk_conf.smart_rev_ramp_time,
					0.0, get_motor_current_limit());	//race throttle torque 2
			utils_truncate_number(&appconf->app_balance_conf.yaw_current_clamp,
					0.0, get_motor_current_limit());	//race throttle torque 3

			utils_truncate_number(&appconf->app_balance_conf.torquetilt_filter,
					0.0, MCCONF_L_MAX_VOLTAGE);	//16smin
			utils_truncate_number(&appconf->app_balance_conf.turntilt_strength,
					0.0, MCCONF_L_MAX_VOLTAGE);	//16smax
			utils_truncate_number(
					&appconf->app_balance_conf.turntilt_angle_limit, 0.0,
					MCCONF_L_MAX_VOLTAGE);	//20smin
			utils_truncate_number(
					&appconf->app_balance_conf.turntilt_start_angle, 0.0,
					MCCONF_L_MAX_VOLTAGE);	//20smax
			utils_truncate_number(
					&appconf->app_balance_conf.torquetilt_off_speed, 0.0,
					MCCONF_L_MAX_VOLTAGE);	//22smin
			utils_truncate_number(
					&appconf->app_balance_conf.torquetilt_strength, 0.0,
					MCCONF_L_MAX_VOLTAGE);	//22max
			utils_truncate_number(&appconf->app_balance_conf.booster_angle, 0.0,
			MCCONF_L_MAX_VOLTAGE);	//24min
			utils_truncate_number(&appconf->app_balance_conf.booster_ramp, 0.0,
			MCCONF_L_MAX_VOLTAGE);	//24max

			conf_general_store_app_configuration(appconf);
			app_set_configuration(appconf);
			timeout_configure(appconf->timeout_msec,
					appconf->timeout_brake_current, appconf->kill_sw_mode);
			chThdSleepMilliseconds(50);
		...
		chThdSleepMilliseconds(50);
		if (change == 1) { //reboot, calibrate motor again fix later
			__disable_irq();
			for (;;) {
			};
	...
	case COMM_GET_APPCONF_DEFAULT: {
		app_configuration *appconf = mempools_alloc_appconf();
		if (appconf->servo_out_enable != get_motor_direction()) {
			appconf->servo_out_enable = get_motor_direction();
			app_set_configuration(appconf);
		}
		if (appconf->app_balance_conf.kd_biquad_lowpass
				!= get_motor_temp_start()) {
			appconf->app_balance_conf.kd_biquad_lowpass =
					get_motor_temp_start();
			app_set_configuration(appconf);
		}
		if (appconf->app_balance_conf.kd_biquad_highpass
				!= get_motor_temp_end()) {
			appconf->app_balance_conf.kd_biquad_highpass = get_motor_temp_end();
			app_set_configuration(appconf);
		}
		...
		if (packet_id == COMM_GET_APPCONF) {
			*appconf = *app_get_configuration();
			...
			/*
			 ...
			 {
			 //	talaria_confgenerator_set_defaults_appconf(appconf);
			 } else if (appconf->imu_conf.mode == 6) //ko rs
			 {
			 //	talaria_confgenerator_set_defaults_appconf(appconf);
			 }
	...
	case COMM_GET_DECODED_PPM: {
		...
		buffer_append_int32(send_buffer,
				(int32_t)(servodec_get_last_pulse_len(0) * 1000000.0), &ind);
		reply_func(send_buffer, ind);
	}
		break;
	...
	case COMM_GET_DECODED_ADC: {
		...
		buffer_append_int32(send_buffer,
				(int32_t)(app_adc_get_voltage() * 1000000.0), &ind);
		buffer_append_int32(send_buffer,
				(int32_t)(app_adc_get_decoded_level2() * 1000000.0), &ind);
		buffer_append_int32(send_buffer,
				(int32_t)(app_adc_get_voltage2() * 1000000.0), &ind);
		reply_func(send_buffer, ind);
	}
		break;
	...
	case COMM_GET_DECODED_CHUK: {
		...
		buffer_append_int32(send_buffer,
				(int32_t)(app_nunchuk_get_decoded_chuk() * 1000000.0), &ind);
		reply_func(send_buffer, ind);
	}
		break;
	...
	case COMM_GET_DECODED_BALANCE: {
		...
		buffer_append_int32(send_buffer,
				(int32_t)(app_balance_get_pid_output() * 1000000.0), &ind);
		buffer_append_int32(send_buffer,
				(int32_t)(app_balance_get_pitch_angle() * 1000000.0), &ind);
		buffer_append_int32(send_buffer,
				(int32_t)(app_balance_get_roll_angle() * 1000000.0), &ind);
		buffer_append_uint32(send_buffer, app_balance_get_diff_time(), &ind);
		buffer_append_int32(send_buffer,
				(int32_t)(app_balance_get_motor_current() * 1000000.0), &ind);
		buffer_append_int32(send_buffer,
				(int32_t)(app_balance_get_debug1() * 1000000.0), &ind);
		...
		buffer_append_int32(send_buffer,
				(int32_t)(app_balance_get_adc1() * 1000000.0), &ind);
		buffer_append_int32(send_buffer,
				(int32_t)(app_balance_get_adc2() * 1000000.0), &ind);
		buffer_append_int32(send_buffer,
				(int32_t)(app_balance_get_debug2() * 1000000.0), &ind);
		reply_func(send_buffer, ind);
	}
		break;
	...
	case COMM_GET_VALUES_SETUP_SELECTIVE: {
		...
		if (mask & ((uint32_t) 1 << 5)) {
			buffer_append_float32(send_buffer, mc_interface_get_rpm(), 1e0,
					&ind);
		}
		if (mask & ((uint32_t) 1 << 6)) {
			buffer_append_float32(send_buffer, mc_interface_get_speed(), 1e3,
					&ind);
		}
		if (mask & ((uint32_t) 1 << 7)) {
			buffer_append_float16(send_buffer,
		...
		if (mask & ((uint32_t) 1 << 13)) {
			buffer_append_float32(send_buffer, mc_interface_get_distance(), 1e3,
					&ind);
		}
		if (mask & ((uint32_t) 1 << 14)) {
			buffer_append_float32(send_buffer, mc_interface_get_distance_abs(),
					1e3, &ind);
		}
		if (mask & ((uint32_t) 1 << 15)) {
			buffer_append_float32(send_buffer, mc_interface_get_pid_pos_now(),
					1e6, &ind);
		}
		...
		if (mask & ((uint32_t) 1 << 20)) {
			buffer_append_uint32(send_buffer, mc_interface_get_odometer(),
					&ind);
		}
	...
	case COMM_GET_MCCONF_TEMP: {
		...
		*mcconf = *mc_interface_get_configuration();
		...
		send_buffer[ind++] = packet_id;
		buffer_append_float32_auto(send_buffer, mcconf->l_current_min_scale,
				&ind);
		buffer_append_float32_auto(send_buffer, mcconf->l_current_max_scale,
				&ind);
		...
		buffer_append_float32_auto(send_buffer, mcconf->si_wheel_diameter,
				&ind);
	...
	case COMM_GET_BATTERY_CUT: {
		...
		send_buffer[ind++] = packet_id;
		buffer_append_float32(send_buffer, mcconf->l_battery_cut_start, 1e3,
				&ind);
		buffer_append_float32(send_buffer, mcconf->l_battery_cut_end, 1e3,
				&ind);
	...
	case COMM_PSW_GET_STATUS: {
		...
		if (stat) {
			...
			buffer_append_float32_auto(send_buffer, UTILS_AGE_S(stat->rx_time),
					&ind);
	...
	case COMM_QMLUI_WRITE: {
		...
		uint16_t flash_res = flash_helper_write_qmlui(qmlui_offset, data + ind,
				len - ind);
	...
	case COMM_IO_BOARD_GET_ALL: {
		...
		if (adc_1_4) {
			send_buffer[ind++] = 1;
			buffer_append_float32_auto(send_buffer,
					UTILS_AGE_S(adc_1_4->rx_time), &ind);
			buffer_append_float16(send_buffer, adc_1_4->adc_voltages[0], 1e2,
					&ind);
			buffer_append_float16(send_buffer, adc_1_4->adc_voltages[1], 1e2,
					&ind);
			buffer_append_float16(send_buffer, adc_1_4->adc_voltages[2], 1e2,
					&ind);
			buffer_append_float16(send_buffer, adc_1_4->adc_voltages[3], 1e2,
					&ind);
		}
		...
		if (adc_5_8) {
			...
			buffer_append_float32_auto(send_buffer,
					UTILS_AGE_S(adc_1_4->rx_time), &ind);
			buffer_append_float16(send_buffer, adc_5_8->adc_voltages[0], 1e2,
					&ind);
			buffer_append_float16(send_buffer, adc_5_8->adc_voltages[1], 1e2,
					&ind);
			buffer_append_float16(send_buffer, adc_5_8->adc_voltages[2], 1e2,
					&ind);
			buffer_append_float16(send_buffer, adc_5_8->adc_voltages[3], 1e2,
					&ind);
		}
		...
		if (digital_in) {
			...
			buffer_append_float32_auto(send_buffer,
					UTILS_AGE_S(adc_1_4->rx_time), &ind);
			buffer_append_uint32(send_buffer,
					(digital_in->inputs >> 32) & 0xFFFFFFFF, &ind);
			buffer_append_uint32(send_buffer,
					(digital_in->inputs >> 0) & 0xFFFFFFFF, &ind);
		}
	...
	case COMM_GET_STATS: {
		...
		if (mask & ((uint32_t) 1 << 0)) {
			buffer_append_float32_auto(send_buffer,
					mc_interface_stat_speed_avg(), &ind);
		}
		if (mask & ((uint32_t) 1 << 1)) {
			buffer_append_float32_auto(send_buffer,
					mc_interface_stat_speed_max(), &ind);
		}
		if (mask & ((uint32_t) 1 << 2)) {
			buffer_append_float32_auto(send_buffer,
					mc_interface_stat_power_avg(), &ind);
		}
		if (mask & ((uint32_t) 1 << 3)) {
			buffer_append_float32_auto(send_buffer,
					mc_interface_stat_power_max(), &ind);
		}
		if (mask & ((uint32_t) 1 << 4)) {
			buffer_append_float32_auto(send_buffer,
					mc_interface_stat_current_avg(), &ind);
		}
		if (mask & ((uint32_t) 1 << 5)) {
			buffer_append_float32_auto(send_buffer,
					mc_interface_stat_current_max(), &ind);
		}
		if (mask & ((uint32_t) 1 << 6)) {
			buffer_append_float32_auto(send_buffer,
					mc_interface_stat_temp_mosfet_avg(), &ind);
		}
		if (mask & ((uint32_t) 1 << 7)) {
			buffer_append_float32_auto(send_buffer,
					mc_interface_stat_temp_mosfet_max(), &ind);
		}
		if (mask & ((uint32_t) 1 << 8)) {
			buffer_append_float32_auto(send_buffer,
					mc_interface_stat_temp_motor_avg(), &ind);
		}
		if (mask & ((uint32_t) 1 << 9)) {
			buffer_append_float32_auto(send_buffer,
					mc_interface_stat_temp_motor_max(), &ind);
		}
		if (mask & ((uint32_t) 1 << 10)) {
			buffer_append_float32_auto(send_buffer,
					mc_interface_stat_count_time(), &ind);
		}
...
void commands_fwd_can_frame(int len, unsigned char *data, uint32_t id,
		bool is_extended) {
	if (len > 8) {
		len = 8;
	}
...
void commands_set_app_data_handler(
		void (*func)(unsigned char *data, unsigned int len)) {
	appdata_func = func;
}

void commands_set_hw_data_handler(
		void (*func)(unsigned char *data, unsigned int len)) {
	hwdata_func = func;
}
...
void commands_send_mcconf(COMM_PACKET_ID packet_id, mc_configuration *mcconf) {
	...
	int32_t len = confgenerator_serialize_mcconf(send_buffer_global + 1,
			mcconf);
...
void commands_send_appconf(COMM_PACKET_ID packet_id, app_configuration *appconf) {
	chMtxLock(&send_buffer_mutex);
	send_buffer_global[0] = packet_id;
	int32_t len = confgenerator_serialize_appconf(send_buffer_global + 1,
			appconf);
	commands_send_packet(send_buffer_global, len + 1);
	chMtxUnlock(&send_buffer_mutex);
}
...
void commands_apply_mcconf_hw_limits(mc_configuration *mcconf) {
	...
	} else if (ctrl_loop_freq
			>= (hw_lim_upper(HW_LIM_FOC_CTRL_LOOP_FREQ) * 0.7)) {
	...
	for (;;) {
		...
		case COMM_DETECT_MOTOR_PARAM: {
			...
			if (!conf_general_detect_motor_param(detect_current, detect_min_rpm,
					...
					&detect_coupling_k, detect_hall_table, &detect_hall_res)) {
			...
			buffer_append_int32(send_buffer,
					(int32_t)(detect_cycle_int_limit * 1000.0), &ind);
			buffer_append_int32(send_buffer,
					(int32_t)(detect_coupling_k * 1000.0), &ind);
		...
		case COMM_DETECT_MOTOR_FLUX_LINKAGE: {
			...
			float linkage;
			bool res = conf_general_measure_flux_linkage(current, duty, min_rpm,
					resistance, &linkage);
		...
		case COMM_DETECT_ENCODER: {
			if (encoder_is_configured()) {
				...
				*mcconf_old = *mcconf;
				...
				static int twist_throttle = 0;
				static int thumb_throttle = 0;
				twist_throttle = appconf->app_adc_conf.ctrl_type;
				thumb_throttle = appconf->send_can_status;

				appconf->app_adc_conf.ctrl_type = 0;
				appconf->send_can_status = 0;
				app_set_configuration(appconf);

				float offset = 0.0;
				float ratio = 0.0;
				bool inverted = false;
				mcpwm_foc_encoder_detect(current, false, &offset, &ratio,
						&inverted);
				//mc_interface_set_configuration(mcconf_old);
				if (offset < 1000) {
				...
				appconf->app_adc_conf.ctrl_type = twist_throttle;
				appconf->send_can_status = thumb_throttle;
				app_set_configuration(appconf);
			...
			chThdSleepMilliseconds(200);    //wait for blocking thread send
			__disable_irq();
			for (;;) {    //reboot from watchdog
			}
		...
		case COMM_DETECT_APPLY_ALL_FOC: {
			...
			int res = conf_general_detect_apply_all_foc_can(detect_can,
					max_power_loss, min_current_in, max_current_in,
