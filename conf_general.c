...
//		mc_interface_fault_stop(FAULT_CODE_FLASH_CORRUPTION_APP_CFG, false, false);
	...
	if (!is_ok) {
		...
		talaria_confgenerator_set_defaults_appconf(conf);
#endif
...
#endif
#if TALARIA
		talaria_confgenerator_set_defaults_mcconf(conf);

#endif

#if KORS
		kors_confgenerator_set_defaults_mcconf(conf);
...
bool conf_general_detect_motor_param(float current, float min_rpm,
		float low_duty, float *int_limit, float *bemf_coupling_k,
		int8_t *hall_table, int *hall_res) {
	...
	*int_limit = mcpwm_read_reset_avg_cycle_integrator();
	...
	float avg_cycle_integrator_running =
			mcpwm_read_reset_avg_cycle_integrator();
	...
	*mcconf_old = *mcconf;
	...
	for (int i = 0; i < 4; i++) {
		...
		while (mc_interface_get_duty_cycle_now() < duty) {
			...
			if (mc_interface_get_duty_cycle_now() >= (duty / 2.0)
					&& !switch_done) {
				mcpwm_switch_comm_mode(COMM_MODE_DELAY);
				switch_done = true;
			}
...
static bool measure_r_l_imax(float current_min, float current_max,
		float max_power_loss, float *r, float *l, float *ld_lq_diff,
		float *i_max) {
	...
	*l = mcpwm_foc_measure_inductance_current(i_last, 100, 0, ld_lq_diff)
			* 1e-6;
	*ld_lq_diff *= 1e-6;
...
int conf_general_detect_apply_all_foc(float max_power_loss,
	...
	*mcconf_old_second = *mcconf_second;
	...
	int res = conf_general_measure_flux_linkage_openloop(i_max / 2.5, 0.3, 1800,
			r, l, &lambda, &lambda_undriven, &lambda_undriven_samples);
	...
	if (res && res_linkage_m2) {
		...
		if (mc_interface_temp_motor_filtered() > -10) {
			mcconf_old->foc_temp_comp_base_temp =
...
int conf_general_detect_apply_all_foc_can(bool detect_can, float max_power_loss,
		float min_current_in, float max_current_in, float openloop_rpm,
		float sl_erpm) {
	...
	if (detect_can) {
		for (int i = 0; i < 255; i++) {
			...
			if (comm_can_ping(i, &hw_type)) {
				...
				comm_can_conf_current_limits_in(i, false,
						mcconf->l_in_current_min, mcconf->l_in_current_max);
