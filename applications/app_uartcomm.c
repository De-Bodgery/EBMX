...
static uint8_t TxGpioPin[UART_NUMBER], RxGpioPin[UART_NUMBER],
		gpioAF[UART_NUMBER];
...
static data_func write_functions[3] = { write_packet_1, write_packet_2,
		write_packet_3 };
static data_func process_functions[3] = { process_packet_1, process_packet_2,
		process_packet_3 };
static data_func send_functions[3] = { send_packet_1, send_packet_2,
		send_packet_3 };
...
void app_uartcomm_start(UART_PORT port_number) {
	...
	if (!thread_is_running) {
		chThdCreateStatic(packet_process_thread_wa,
	...
	palSetPadMode(TxGpioPort[port_number], TxGpioPin[port_number],
			PAL_MODE_ALTERNATE(gpioAF[port_number]) | PAL_STM32_OSPEED_HIGHEST
...
void app_uartcomm_configure(uint32_t baudrate, bool enabled,
		UART_PORT port_number) {
	if (port_number >= UART_NUMBER) {
		return;
	}
...
/*
	...
	for (int port_number = 0; port_number < UART_NUMBER; port_number++) {
		chEvtRegisterMaskWithFlags(&(*serialPortDriverRx[port_number]).event,
				&el[port_number], EVENT_MASK(0), CHN_INPUT_AVAILABLE);
	}
*/
	event_listener_t elp;
		chEvtRegisterMaskWithFlags(&SD4.event, &elp, EVENT_MASK(0),
		CHN_INPUT_AVAILABLE);
	...
	for (;;) {
		...
		while (rx) {
			...
			if (res != MSG_TIMEOUT) {
			packet_process_byte(res, &packet_state[1]);
				//packet_process_byte(res, 2);
				rx = true;
			}
			//}
			//}
