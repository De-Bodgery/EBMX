...
void app_pas_start(bool is_primary_output) {
	stop_now = false;
	chThdCreateStatic(pas_thread_wa, sizeof(pas_thread_wa), NORMALPRIO,
			pas_thread, NULL);
...
void pas_event_handler(void) {
	...
	const float timestamp =
			(float) chVTGetSystemTimeX() / (float) CH_CFG_ST_FREQUENCY;
	...
	for (;;) {
		...
		case PAS_CTRL_TYPE_CADENCE:
			...
			if (config.pedal_rpm_end > (config.pedal_rpm_start + 1.0)) {
				output = utils_map(pedal_rpm, config.pedal_rpm_start,
						config.pedal_rpm_end, 0.0,
						config.current_scaling * sub_scaling);
				utils_truncate_number(&output, 0.0,
						config.current_scaling * sub_scaling);
			} else {
		...
		float ramp_time =
				fabsf(output) > fabsf(output_ramp) ?
		...
		if (ramp_time > 0.01) {
			...
			utils_truncate_number(&output_ramp, 0.0,
					config.current_scaling * sub_scaling);
