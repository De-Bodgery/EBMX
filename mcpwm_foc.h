...
extern uint16_t i_buf_ptr;
#define i_buf_size 2048
#define i_buf_mask 0x7FF
//static volatile uint8_t i_buf_ptr=0;
extern float ia_buf[i_buf_size];
extern float ib_buf[i_buf_size];
extern float ic_buf[i_buf_size];

extern uint16_t vi_buf_ptr;
#define vi_buf_mask 0x1FF
extern volatile float vd_buf[512];
extern volatile float vq_buf[512];
extern volatile float id_buf[512];
extern volatile float iq_buf[512];

#endif
void update_phase_cal(void);
float mcpwm_foc_get_ia(void);
float mcpwm_foc_get_ib(void);
float mcpwm_foc_get_ic(void);
