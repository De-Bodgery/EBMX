...
void mc_interface_init(void) {
	...
	conf_general_read_mc_configuration((mc_configuration*) &m_motor_1.m_conf,
			false);
#ifdef HW_HAS_DUAL_MOTORS
	...
	chThdCreateStatic(stat_thread_wa, sizeof(stat_thread_wa), NORMALPRIO,
			stat_thread, NULL);
...
//only check hall sensor once on startup
void hall_sensor_error_check_once(void) {

	if (motor_now()->m_conf.foc_sensor_mode == FOC_SENSOR_MODE_HALL) {
		static int hall;
		hall = utils_read_hall(false, 10);

		if (hall == 0 || hall == 7) {		//000 or 111

			mc_interface_fault_stop(FAULT_CODE_HALL_SENSOR, false, false);
...
void mc_interface_set_configuration(mc_configuration *configuration) {
	...
	/*
	 if (configuration->m_sensor_port_mode == SENSOR_PORT_MODE_SINCOS) {
	 encoder_init_sincos(motor->m_conf.foc_encoder_sin_gain,
	 motor->m_conf.foc_encoder_sin_offset,
	 motor->m_conf.foc_encoder_cos_gain,
	 motor->m_conf.foc_encoder_cos_offset,
...
void get_sincos(void) {

	float sin = (ENCODER_SIN_VOLTS - motor_now()->m_conf.foc_encoder_sin_offset)
			* motor_now()->m_conf.foc_encoder_sin_gain;
	float cos = (ENCODER_COS_VOLTS - motor_now()->m_conf.foc_encoder_cos_offset)
			* motor_now()->m_conf.foc_encoder_cos_gain;

	float angle_tmp = RAD2DEG_f(utils_fast_atan2(sin, cos)) + 180;
	commands_printf(" %.2f ", angle_tmp);
}
...
void update_battery(float min, float max) {

	motor_now()->m_conf.l_min_vin = min;
	motor_now()->m_conf.l_max_vin = max;

}

void process_speed_limit(float km_speed) {
	motor_now()->m_conf.l_max_erpm = (((km_speed / 3.6)
			* motor_now()->m_conf.si_gear_ratio)
			/ (M_PI * motor_now()->m_conf.si_wheel_diameter)) * 60
			* (motor_now()->m_conf.si_motor_poles / 2.0);
}
void update_wattage_current_regen(float watt, float motor_current,
		float battery_current, float regen_current)

{

	motor_now()->m_conf.l_watt_max = watt * 1000;
	motor_now()->m_conf.l_current_max = motor_current;
	motor_now()->m_conf.l_in_current_max = battery_current;
	motor_now()->m_conf.l_current_min = regen_current * -1;

}
void update_gear_wheel(float mm, float ratio) {

	motor_now()->m_conf.si_wheel_diameter = mm / (float) 1000;
	motor_now()->m_conf.si_gear_ratio = ratio;

}

void invert_motor_direction(int direction) {
	motor_now()->m_conf.m_invert_direction = direction;

}

int get_motor_direction(void) {
	return motor_now()->m_conf.m_invert_direction;
}

float get_motor_temp_start(void) {
	return motor_now()->m_conf.l_temp_motor_start;
}
float get_motor_temp_end(void) {
	return motor_now()->m_conf.l_temp_motor_end;
}
float get_motor_temp_accel(void) {
	return motor_now()->m_conf.l_temp_accel_dec;
}
void set_motor_temp_start(float temp) {
	motor_now()->m_conf.l_temp_motor_start = temp;

}
void set_motor_temp_end(float temp) {
	motor_now()->m_conf.l_temp_motor_end = temp;

}
void set_motor_temp_accel(float percentage) {
	motor_now()->m_conf.l_temp_accel_dec = percentage / 100;
}

void change_ntc(void) {
	motor_now()->m_conf.m_motor_temp_sens_type = TEMP_SENSOR_KTY84_130;

	app_configuration *appconf = mempools_alloc_appconf();
	conf_general_read_app_configuration(appconf);

	appconf->app_ppm_conf.safe_start = 1; // temperature sensor on
...
void changetome1803(void) {

	//motor_now()->m_conf.l_watt_max = 150000;
	//motor_now()->m_conf.l_current_max = 120;

	//motor_now()->m_conf.foc_sensor_mode = FOC_SENSOR_MODE_HALL;
	motor_now()->m_conf.foc_sensor_mode = FOC_SENSOR_MODE_SENSORLESS;
	motor_now()->m_conf.foc_motor_r = 0.0027;
	motor_now()->m_conf.foc_motor_l = 0.00002049;
	motor_now()->m_conf.foc_motor_flux_linkage = 0;
	motor_now()->m_conf.foc_current_kp = 0.0295;
	motor_now()->m_conf.foc_current_ki = 2.85;
	motor_now()->m_conf.foc_observer_gain = 2.57e6;

}
float get_motor_current_limit(void) {

	return motor_now()->m_conf.si_motor_nl_current;

}
void changetowa(void) {
	...
	//motor_now()->m_conf.foc_sensor_mode = FOC_SENSOR_MODE_HALL;
	motor_now()->m_conf.foc_sensor_mode = FOC_SENSOR_MODE_HALL;
	motor_now()->m_conf.foc_motor_r = 0.0029;
	motor_now()->m_conf.foc_motor_l = 0.00002954;
	motor_now()->m_conf.foc_motor_flux_linkage = 0.019721;
	motor_now()->m_conf.foc_current_kp = 0.0295;
	motor_now()->m_conf.foc_current_ki = 2.85;
	motor_now()->m_conf.foc_observer_gain = 2.57e6;
	motor_now()->m_conf.foc_hall_table[0] = 255;
	motor_now()->m_conf.foc_hall_table[1] = 33;
	motor_now()->m_conf.foc_hall_table[2] = 86;
	motor_now()->m_conf.foc_hall_table[3] = 52;
	motor_now()->m_conf.foc_hall_table[4] = 178;
	motor_now()->m_conf.foc_hall_table[5] = 197;
	motor_now()->m_conf.foc_hall_table[6] = 135;
	motor_now()->m_conf.foc_hall_table[7] = 255;

}
void update_fw_sensor(float fw_per, int sensor) {

	motor_now()->m_conf.foc_fw_current_max = 250 * (fw_per / 100); //250A*percentage
	if (sensor == 0)
		motor_now()->m_conf.foc_sensor_mode = FOC_SENSOR_MODE_SENSORLESS;
...
const char* mc_interface_fault_to_string(mc_fault_code fault) {
	...
	case FAULT_CODE_HALL_SENSOR:
		return "FAULT_CODE_HALL_SENSOR";

	case FAULT_CODE_THROTTLE_VOLTAGE:
		return "FAULT_CODE_THROTTLE_VOLTAGE";
	case FAULT_CODE_DISPLAY:
		return "FAULT_CODE_DISPLAY";

	case FAULT_CODE_REGEN_VOLTAGE:
		return "FAULT_CODE_REGEN_VOLTAGE";
...
void rs_handler(int set) {
	app_configuration *appconf = mempools_alloc_appconf();
	conf_general_read_app_configuration(appconf);

	if (set == 0) { // set6=0 means set to street,median_filter =0 means settings lock is disabled
		appconf->app_pas_conf.sensor_type = 0;

	} else if (set == 1) { // set6=1 =race
		appconf->app_pas_conf.sensor_type = 1;
...
void oqc_loading_test(void) {
	app_configuration *appconf = mempools_alloc_appconf();
	conf_general_read_app_configuration(appconf);
	appconf->permanent_uart_enabled = 0; //no switch
	appconf->app_chuk_conf.ctrl_type = 4; //battery series bypass
	appconf->app_balance_conf.booster_current = 0; //no fw current
	appconf->app_ppm_conf.safe_start = 0; //no temperature sensor
	appconf->app_adc_conf.ctrl_type = 1; //throttle only
	appconf->send_can_status = 0; //noregen
	appconf->app_balance_conf.torquetilt_on_speed = 400; //race mode battery current
	appconf->app_balance_conf.torquetilt_angle_limit = 400; //street mode battery current
	appconf->app_balance_conf.yaw_current_clamp = 600; //gear3 street motor current
	app_set_configuration(appconf);
//conf_general_store_app_configuration(appconf);s
	mempools_free_appconf(appconf);

}

void oqc_current_test(void) {
	app_configuration *appconf = mempools_alloc_appconf();
	conf_general_read_app_configuration(appconf);
	appconf->permanent_uart_enabled = 0; //no switch
	appconf->app_chuk_conf.ctrl_type = 4; //battery series bypass
	appconf->app_balance_conf.booster_current = 0; //no fw current
	appconf->app_ppm_conf.safe_start = 0; //no temperature sensor
	appconf->app_adc_conf.ctrl_type = 0; //no throttle
	appconf->send_can_status = 0; //noregen
	appconf->app_balance_conf.torquetilt_on_speed = 400; //race mode battery current
	appconf->app_balance_conf.torquetilt_angle_limit = 400; //street mode battery current
	appconf->app_balance_conf.yaw_current_clamp = 900; //gear3 street motor current
...
void set_rs_app() {
	...
	if (check_display_present()) {
		...
		} else if (!app_get_rs_mode()) { //toggle
			appconf->app_pas_conf.sensor_type = 1;
...
void set_display_config(volatile int set5, volatile int set6,
		volatile int set10) {

	app_configuration *appconf = mempools_alloc_appconf();
	conf_general_read_app_configuration(appconf);

	if (((set5 & (1 << 0)) >> 0) == 0) { //bbrs 0 bit =0   C
		appconf->app_pas_conf.ctrl_type = 0;

	} else if (((set5 & (1 << 0)) >> 0) == 1) { //F
		appconf->app_pas_conf.ctrl_type = 1;

	}

	if (((set5 & (1 << 1)) >> 1) == 0) { //bbrs 1 bit =0   KM
		appconf->app_pas_conf.invert_pedal_direction = 0;
		//commands_printf("km");
	} else if (((set5 & (1 << 1)) >> 1) == 1) {
		appconf->app_pas_conf.invert_pedal_direction = 1;
		//commands_printf("miles");
	}

	if (set5 >> 4 == 0) { //battery cells
		//commands_printf("16");
		appconf->app_chuk_conf.ctrl_type = 0;
	} else if (set5 >> 4 == 1) {
		//commands_printf("20");
		appconf->app_chuk_conf.ctrl_type = 1;
	} else if (set5 >> 4 == 2) {
		//commands_printf("22");
		appconf->app_chuk_conf.ctrl_type = 2;
	} else if (set5 >> 4 == 3) {
		//commands_printf("24");
		appconf->app_chuk_conf.ctrl_type = 3;
	}
	if (set6 == 0x00) //&& appconf->app_ppm_conf.median_filter == 0)
			{ // set6=0 means set to street,median_filter =0 means settings lock is disabled
		appconf->app_pas_conf.sensor_type = 0;

	} else if (set6 == 0x01) // && appconf->app_ppm_conf.median_filter == 0)
			{ // set6=1 =race
		appconf->app_pas_conf.sensor_type = 1;

	}
	if (((set10 & (1 << 0)) >> 0) == 0) { //stand sensor off
		appconf->app_chuk_conf.throttle_exp_mode = 0;
	}
	if (((set10 & (1 << 0)) >> 0) == 1) {
		appconf->app_chuk_conf.throttle_exp_mode = 1;
	}
	if (((set10 & (1 << 4)) >> 4) == 0) { //brake sensor off
		appconf->app_nrf_conf.send_crc_ack = 0;
	}
	if (((set10 & (1 << 4)) >> 4) == 1) {
		appconf->app_nrf_conf.send_crc_ack = 1;
	}
...
void mc_interface_set_current_rel(float val) {
	...
	mc_interface_set_current(
			val * motor_now()->m_conf.lo_current_motor_max_now);
}
...
void high_current_sampling(void) {
	if (mc_interface_read_reset_avg_motor_current() >= 750)
		motor_now()->m_conf.foc_sample_high_current = 1;
	else
		motor_now()->m_conf.foc_sample_high_current = 0;
}
...
float mc_interface_read_reset_avg_id(void) {
	float res = motor_now()->m_motor_id_sum
			/ motor_now()->m_motor_id_iterations;
...
float mc_interface_read_reset_avg_iq(void) {
	float res = motor_now()->m_motor_iq_sum
			/ motor_now()->m_motor_iq_iterations;
...
float mc_interface_read_reset_avg_vd(void) {
	float res = motor_now()->m_motor_vd_sum
			/ motor_now()->m_motor_vd_iterations;
...
float mc_interface_read_reset_avg_vq(void) {
	float res = motor_now()->m_motor_vq_sum
			/ motor_now()->m_motor_vq_iterations;
...
float mc_interface_get_battery_level(float *wh_left) {
	...
	case BATTERY_TYPE_LIION_3_0__4_2:
		battery_avg_voltage = ((3.2 + 4.2) / 2.0)
				* (float) (conf->si_battery_cells);
		battery_avg_voltage_left = ((3.2 * (float) (conf->si_battery_cells)
				+ v_in) / 2.0);
	...
	case BATTERY_TYPE_LIIRON_2_6__3_6:
		battery_avg_voltage = ((2.8 + 3.6) / 2.0)
				* (float) (conf->si_battery_cells);
		battery_avg_voltage_left = ((2.8 * (float) (conf->si_battery_cells)
				+ v_in) / 2.0);
		ah_left = utils_map(v_in / (float) (conf->si_battery_cells), 2.6, 3.6,
				0.0, conf->si_battery_ah);
		break;

	case BATTERY_TYPE_LEAD_ACID:
		// TODO: This does not really work for lead-acid batteries
		battery_avg_voltage = ((2.1 + 2.36) / 2.0)
				* (float) (conf->si_battery_cells);
		battery_avg_voltage_left = ((2.1 * (float) (conf->si_battery_cells)
				+ v_in) / 2.0);
		ah_left = utils_map(v_in / (float) (conf->si_battery_cells), 2.1, 2.36,
				0.0, conf->si_battery_ah);
		break;
...
float mc_interface_get_actual_rpm(void) {
	const volatile mc_configuration *conf = mc_interface_get_configuration();
	const float rpm = abs(mc_interface_get_rpm())
			/ (conf->si_motor_poles / 2.0);
	return rpm;
}
...
//ledpwm_update_pwm();
	...
	float input_voltage = GET_INPUT_VOLTAGE(); //const
...
// Check for faults that should stop the motor
	...
	if (voltage_diff_now > 1.0e-3) {
		...
		if (wrong_voltage_integrator > max_voltage) {
			...
			if (input_voltage > conf_now->l_min_vin)
				mc_interface_fault_stop(FAULT_CODE_OVER_VOLTAGE,
						is_second_motor, true);
			else {
				if (slow_voltage_cutout()) {
					cutout_timeout++;
					if (cutout_timeout > 30000)
						mc_interface_fault_stop(FAULT_CODE_UNDER_VOLTAGE,
								is_second_motor, true);
				} else
					mc_interface_fault_stop(FAULT_CODE_UNDER_VOLTAGE,
							is_second_motor, true);
...
// functions. That will make this interrupt run a bit faster.
	...
	if (conf_now->motor_type == MOTOR_TYPE_FOC) {
		...
		current_filtered = mcpwm_foc_get_tot_current_filtered_motor(
				is_second_motor);
		...
		if (a >= m_sample_int) {
			...
			if (conf_now->motor_type == MOTOR_TYPE_FOC) {
				...
				m_phase_samples[m_sample_now] = (uint8_t)(
						mcpwm_foc_get_phase() / 360.0 * 250.0);
...
//				m_phase_samples[m_sample_now] = (uint8_t)(ang / 360.0 * 250.0);
			...
			if (state == MC_STATE_DETECTING) {
				m_curr0_samples[m_sample_now] =
						(int16_t) mcpwm_detect_currents[mcpwm_get_comm_step()
								- 1];
				m_curr1_samples[m_sample_now] =
						(int16_t) mcpwm_detect_currents_diff[mcpwm_get_comm_step()
								- 1];

				m_ph1_samples[m_sample_now] =
						(int16_t) mcpwm_detect_voltages[0];
				m_ph2_samples[m_sample_now] =
						(int16_t) mcpwm_detect_voltages[1];
				m_ph3_samples[m_sample_now] =
						(int16_t) mcpwm_detect_voltages[2];
			} else {
				if (is_second_motor) {
			...
			m_last_adc_duration_sample =
					mc_interface_get_last_inj_adc_isr_duration();
		}
	}
}
...
static void update_override_limits(volatile motor_if_state_t *motor,
	...
	UTILS_LP_FAST(motor->m_temp_fet,
			NTC_TEMP(is_motor_1 ? ADC_IND_TEMP_MOS : ADC_IND_TEMP_MOS_M2), 0.1);
	...
	case TEMP_SENSOR_NTC_10K_25C:
		temp_motor =
				is_motor_1 ?
						NTC_TEMP_MOTOR(conf->m_ntc_motor_beta) :
	...
	case TEMP_SENSOR_NTC_100K_25C:
		temp_motor =
				is_motor_1 ?
						NTC100K_TEMP_MOTOR(conf->m_ntc_motor_beta) :
	...
	case TEMP_SENSOR_KTY83_122: {
		...
		float res = NTC_RES_MOTOR(ADC_Value[is_motor_1 ? ADC_IND_TEMP_MOTOR :
	...
	case TEMP_SENSOR_KTY84_130: {
		...
		temp_motor = -7.82531699e-12 * res * res * res * res
				+ 6.34445902e-8 * res * res * res - 0.00020119157 * res * res
				+ 0.407683016 * res - 161.357536;
	}
		break;
	}

	if (get_ntc() == 0)
		temp_motor = motor_now()->m_temp_fet;
...
// Temperature MOSFET
	...
	if (motor->m_temp_fet < (conf->l_temp_fet_start + 0.1)) {
		...
		maxc = utils_map(motor->m_temp_fet, conf->l_temp_fet_start,
				conf->l_temp_fet_end, maxc, 0.0);
...
// Temperature MOTOR
	...
	if (motor->m_temp_motor < (conf->l_temp_motor_start + 0.1)) {
		...
		maxc = utils_map(motor->m_temp_motor, conf->l_temp_motor_start,
				conf->l_temp_motor_end, maxc, 0.0);
...
// in order to still have braking torque available
	const float temp_fet_accel_start = utils_map(conf->l_temp_accel_dec, 0.0,
			1.0, conf->l_temp_fet_start, 25.0);
	const float temp_fet_accel_end = utils_map(conf->l_temp_accel_dec, 0.0, 1.0,
			conf->l_temp_fet_end, 25.0);
	const float temp_motor_accel_start = utils_map(conf->l_temp_accel_dec, 0.0,
			1.0, conf->l_temp_motor_start, 25.0);
	const float temp_motor_accel_end = utils_map(conf->l_temp_accel_dec, 0.0,
			1.0, conf->l_temp_motor_end, 25.0);
	...
	if (motor->m_temp_motor < (temp_motor_accel_start + 0.1)) {
		...
		lo_motor_temp_accel = utils_map(motor->m_temp_motor,
				temp_motor_accel_start, temp_motor_accel_end, l_current_max_tmp,
				0.0);
	}
...
// Duty max
	...
	if (duty_now_abs < (conf->l_duty_start * conf->l_max_duty)
		...
		lo_max_duty = utils_map(duty_now_abs,
				(conf->l_duty_start * conf->l_max_duty), conf->l_max_duty,
				l_current_max_tmp, 0.0);
	}
...
// BMS limits
	bms_update_limits(&lo_in_min, &lo_in_max, conf->l_in_current_min,
...
static void run_timer_tasks(volatile motor_if_state_t *motor) {
	...
	UTILS_LP_FAST(motor->m_input_voltage_filtered_slower,
			motor->m_input_voltage_filtered, 0.01);
...
// Decrease fault iterations
	...
	if (is_motor_1 ? IS_DRV_FAULT() : IS_DRV_FAULT_2()) {
		...
		if (motor->m_drv_fault_iterations
				>= motor->m_conf.m_fault_stop_time_ms) {
			HW_RESET_DRV_FAULTS();
...
// Relevant only in FOC mode with encoder enabled
	...
	if (motor->m_conf.motor_type == MOTOR_TYPE_FOC
		...
		mc_interface_fault_stop(FAULT_CODE_ENCODER_NO_MAGNET, !is_motor_1,
				false);
	}
	...
	if (motor->m_conf.motor_type == MOTOR_TYPE_FOC
		...
		if (encoder_sincos_get_signal_below_min_error_rate() > 0.05) { /*mc_interface_fault_stop(
		 FAULT_CODE_ENCODER_SINCOS_BELOW_MIN_AMPLITUDE, !is_motor_1,
		 false);*/
		}
		if (encoder_sincos_get_signal_above_max_error_rate() > 0.05) {/*	mc_interface_fault_stop(
		 FAULT_CODE_ENCODER_SINCOS_ABOVE_MAX_AMPLITUDE, !is_motor_1,
		 false);*/
		}
	}
	...
	if (motor->m_conf.motor_type == MOTOR_TYPE_FOC
			&& motor->m_conf.foc_sensor_mode == FOC_SENSOR_MODE_ENCODER
			&& motor->m_conf.m_sensor_port_mode
...
#else
		mcpwm_foc_get_current_offsets(&curr0_offset, &curr1_offset,
				&curr2_offset, false);
#endif
...
// Monitor currents balance. The sum of the 3 currents should be zero
	...
	if (!motor->m_conf.foc_sample_high_current) { // This won't work when high current sampling is used
		...
		if (fabsf(motor->m_motor_current_unbalance)
				> fabsf(MCCONF_MAX_CURRENT_UNBALANCE)) {
			UTILS_LP_FAST(motor->m_motor_current_unbalance_error_rate, 1.0,
					(1 / 1000.0));
		} else {
			UTILS_LP_FAST(motor->m_motor_current_unbalance_error_rate, 0.0,
					(1 / 1000.0));
		}
...
static void update_stats(volatile motor_if_state_t *motor) {
	...
	const double power = mc_interface_get_input_voltage_filtered()
			* fabsf(val.current_in_tot);
		...
		for (int i = 0; i < len; i++) {
			...
			if (m_sample_raw) {
				buffer_append_float32_auto(buffer,
						(float) m_curr0_samples[ind_samp], &index);
				buffer_append_float32_auto(buffer,
						(float) m_curr1_samples[ind_samp], &index);
				buffer_append_float32_auto(buffer,
						(float) m_ph1_samples[ind_samp], &index);
				buffer_append_float32_auto(buffer,
						(float) m_ph2_samples[ind_samp], &index);
				buffer_append_float32_auto(buffer,
						(float) m_ph3_samples[ind_samp], &index);
				buffer_append_float32_auto(buffer,
						(float) m_vzero_samples[ind_samp], &index);
				buffer_append_float32_auto(buffer,
						(float) m_curr_fir_samples[ind_samp], &index);
			} else {
				buffer_append_float32_auto(buffer,
				...
				buffer_append_float32_auto(buffer,
						((float) m_vzero_samples[ind_samp] / 4096.0 * V_REG)
								* ((VIN_R1 + VIN_R2) / VIN_R2)
								* ADC_VOLTS_INPUT_FACTOR, &index);
				buffer_append_float32_auto(buffer,
						(float) m_curr_fir_samples[ind_samp]
								/ (8.0 / FAC_CURRENT), &index);
			}

			buffer_append_float32_auto(buffer,
					(float) m_f_sw_samples[ind_samp] * 10.0, &index);
